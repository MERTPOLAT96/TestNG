...
public class _01_WishList extends BaseDriver {
    ...
    void addToWishList(String text){
        ...
        int randomchoice= Tools.RandomGenerator(searchResult.size()); // 0,1,2,3
        String wishItemText=searchResult.get(randomchoice).getText(); // tıklatılacak elemanın ismini aldım

        List<WebElement> wishList=driver.findElements(By.xpath("//button[@data-original-title='Add to Wish List']"));
        wishList.get(randomchoice).click(); // random elementin wish listine tıklattım
        ...
        //wishItemText i wishTableNames in gettext lerinde varmı diye kontrol edeceğim
